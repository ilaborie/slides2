...
class PrismJsPlugin(
    ...
    private val version: String = "1.16.0",
    ...
    private val plugins: List<String> =
        (if (showLines) listOf("line-numbers") else emptyList()) +
                (if (lineHighlight) listOf("line-highlight") else emptyList())
    ...
    override fun stylesheets(): List<Stylesheet> =
        (listOf("$cloudfare/prism/$version/themes/prism.min.css") +
                // Theme
                (if (theme != null) listOf("$cloudfare/prism/$version/themes/prism-$theme.min.css") else emptyList()) +
                // Plugins
